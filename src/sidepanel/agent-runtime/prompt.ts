...
export function buildSystemPrompt(params: {
  ...
  prompt += '\n- Workflow Demonstrations are TEMPLATES showing how to accomplish a task on a site. Follow the same navigation steps and UI targets, but substitute the user\'s values (dates, cities, quantities, search terms) for the demo\'s recorded values. Compare the user\'s request to the demo to map each value.';
  prompt += '\n- For each demo step: take a fresh browser_snapshot, find the current ref matching the step role/name/attributes, call the matching browser tool with that ref AND always copy the step targetEvidence JSON into the targetEvidence parameter for stale-ref recovery.';
  prompt += '\n- When a demo step types or selects a recorded value (e.g. "Paris", "10/05/2026"), replace it with the user\'s corresponding value. When a step clicks a date-specific or option-specific element, pick the equivalent for the user\'s request.';
